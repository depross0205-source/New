...
async function fetchOHLCV(s, interval, range) {
  ...
  var adjArr = ch.indicators && ch.indicators.adjclose && ch.indicators.adjclose[0] && ch.indicators.adjclose[0].adjclose;
  var allowCloseFallback = (s.c && s.c.charAt(0) === '^'); // 指數/利率資料沒有 adjclose 時才允許 close

  var bars = ch.timestamp.map(function(ts, i) {
    if (!q) return null;
    var close = q.close ? q.close[i] : null;
    var adj = adjArr ? adjArr[i] : null;
    var useAdj = (adj !== undefined && adj !== null && isFinite(adj));

    // 對股票/ETF 強制使用 Yahoo adjclose；沒有 adjclose 就丟掉該筆，避免混用未還原價。
    // ^TNX / ^TWII 等指數型資料才允許 close fallback，僅供 benchmark / hurdle。
    if (!useAdj && !allowCloseFallback) return null;
    var c = useAdj ? adj : close;
    ...
    return {
      ...
      adj: useAdj ? adj : null,
      c: c,
      priceType: useAdj ? 'adjclose' : 'close-fallback-index',
      v: (q.volume && q.volume[i]) || 0
    };
  }).filter(function(v){ return v && v.c != null && isFinite(v.c); });

  if (!bars.length) throw new Error('No adjusted data for ' + tk);
  return bars;
}
...
function shiftYM(ym, offset) {
  ...
  var d = new Date(Date.UTC(y, m - 1 + offset, 1));
  return d.toISOString().slice(0, 7);
}
function getMonthEndBar(bars, ym) {
  if (!bars || !bars.length || !ym) return null;
  var best = null;
  for (var i = 0; i < bars.length; i++) {
    if (bars[i].date && bars[i].date.slice(0, 7) === ym) best = bars[i];
    else if (best && bars[i].date && bars[i].date.slice(0, 7) > ym) break;
  }
  return best;
}
function calcAdjMonthReturn(code, prevDate, currDate) {
  var bars = DAILY[code];
  if (!bars || !bars.length) return {ret:null, note:'No DAILY data'};
  var prevYM = ymFromDate(prevDate);
  var currYM = ymFromDate(currDate);
  var b0 = getMonthEndBar(bars, prevYM);
  var b1 = getMonthEndBar(bars, currYM);
  if (!b0 || !b1) return {ret:null, note:'Missing month-end bar', prevYM:prevYM, currYM:currYM};
  if (!b0.c || !b1.c || b0.c <= 0 || b1.c <= 0) return {ret:null, note:'Invalid month-end price', prevYM:prevYM, currYM:currYM};
  return {
    ret: b1.c / b0.c - 1,
    p0: b0.c,
    p1: b1.c,
    ...
    prevYM: prevYM,
    currYM: currYM,
    priceType0: b0.priceType || 'adjclose',
    priceType1: b1.priceType || 'adjclose',
    method: 'adj month-end per ticker'
  };
}
function isSuspiciousReturn(ret) {
  return ret !== null && isFinite(ret) && Math.abs(ret) >= 0.30;
}
function buildReconcileRows(records, init) {
  var rows = [];
  if (!records || !records.length) return rows;
  records.forEach(function(r, i) {
    var prevNav = i > 0 ? records[i - 1].nav : init;
    var monthGross = 0;
    var monthRecon = 0;
    var hasRecon = false;
    Object.keys(r.stockRets || {}).forEach(function(code) {
      var sr = r.stockRets[code] || {};
      var w = sr.w || 0;
      var ret = (sr.ret === undefined ? null : sr.ret);
      var contrib = (ret === null ? null : w * ret);
      if (contrib !== null && isFinite(contrib)) monthRecon += contrib;
      if (contrib !== null && isFinite(contrib)) hasRecon = true;
      rows.push({
        month: r.month,
        code: code,
        ...
        ret: ret,
        contrib: contrib,
        pnl: contrib === null ? null : prevNav * contrib,
        p0: sr.p0,
        p1: sr.p1,
        d0: sr.d0,
        d1: sr.d1,
        method: sr.method || '',
        note: sr.note || '',
        flag: sr.note || (isSuspiciousReturn(ret) ? 'Large single-name return' : '')
      });
    });
    rows.push({
      month: r.month,
      code: '__MONTH_TOTAL__',
      name: 'Month total before friction check',
      weight: '',
      ret: r.pRet,
      contrib: hasRecon ? monthRecon : null,
      pnl: null,
      p0: '', p1: '', d0: '', d1: '', method: 'sum(weight*ret)',
      note: 'Net return includes friction; diff = pRet - gross contribution',
      flag: (hasRecon && Math.abs(r.pRet - monthRecon) > 0.02) ? 'Diff > 2pp vs net return' : ''
    });
  });
  return rows;
}
...
function runBTcore(mh, mode, opts) {
  ...
  for (var idx=cS; idx<=cE; idx++) {
    ...
    Object.keys(target).forEach(function(c){
      if (c==='CASH') {
        stockRets[c]={ret:cashRet,w:target[c],method:'cash proxy'};
      } else {
        // 對帳版：月報酬一律用「個別標的自己的前月月底最後交易日 → 本月月底最後交易日」adjclose 計算。
        // 這會讓 CSV 的月份績效與 Excel 定義一致，避免跨市場月底交易日不同造成偏差。
        var mr = calcAdjMonthReturn(c, prevM, sigM);
        stockRets[c]={
          ret: mr.ret,
          w: target[c],
          p0: mr.p0,
          ...
          d1: mr.d1,
          method: mr.method,
          note: mr.note || (isSuspiciousReturn(mr.ret) ? 'Large single-name return' : ''),
          priceType0: mr.priceType0,
          priceType1: mr.priceType1
        };
      }
    });
    ...
    if (r.stockRets) {
      Object.keys(r.stockRets).forEach(function(k){
        ...
        detailRows+='<tr style="background:var(--bg);opacity:0.85;">'
          ...
          +((ret===null||ret===undefined)?'--':((ret>=0?'+':'')+(ret*100).toFixed(2)+'%'))
          +(sr.d0&&sr.d1?'<div style="font-size:9px;color:var(--mu)">'+sr.d0+' → '+sr.d1+'</div>':'')
          +(sr.p0&&sr.p1?'<div style="font-size:9px;color:var(--mu)">'+Number(sr.p0).toFixed(2)+' → '+Number(sr.p1).toFixed(2)+'</div>':'')
          +(sr.note?'<div style="font-size:9px;color:var(--re)">'+sr.note+'</div>':'')
          +'</td>'
...
function dlBtAuditCsv() {
  if (!BT_RESULT || !BT_RESULT.records) { sl('btLog','Run backtest first',false); return; }
  var rows = buildReconcileRows(BT_RESULT.records, BT_RESULT.initial);
  var head = ['Month','Code','Name','Weight%','AdjReturn%','Contribution%','PnL','PrevME_Date','CurrME_Date','PrevAdjPrice','CurrAdjPrice','Method','Note','Flag'];
  var out = [head.join(',')];
  rows.forEach(function(r){
    out.push([
      ...
      r.code,
      r.name,
      (r.weight === '' ? '' : (r.weight*100).toFixed(4)),
      (r.ret === null || r.ret === undefined ? '' : (r.ret*100).toFixed(6)),
      (r.contrib === null || r.contrib === undefined ? '' : (r.contrib*100).toFixed(6)),
      (r.pnl === null || r.pnl === undefined ? '' : Math.round(r.pnl)),
      r.d0 || '', r.d1 || '',
      (r.p0 === null || r.p0 === undefined ? '' : r.p0),
      (r.p1 === null || r.p1 === undefined ? '' : r.p1),
      r.method || '', r.note || '', r.flag || ''
    ].map(escCsv).join(','));
  });
  dlText(out.join('\n'),'V1.9_Backtest_Audit_'+new Date().toISOString().slice(0,10)+'.csv','text/csv;charset=utf-8');
  sl('btLog','Audit CSV downloaded: each holding return uses adjusted month-end price.',true);
}
function showAuditSummary() {
  if (!BT_RESULT || !BT_RESULT.records) { sl('btLog','Run backtest first',false); return; }
  var rows = buildReconcileRows(BT_RESULT.records, BT_RESULT.initial);
  var flagged = rows.filter(function(r){ return r.flag; });
  console.table(rows);
  if (flagged.length) console.warn('Audit flags', flagged);
  alert('對帳完成：共 '+rows.length+' 筆明細，異常標記 '+flagged.length+' 筆。詳細請看 console，或下載 Audit CSV。');
}
